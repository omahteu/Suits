        ...
        if (seAutomatica) {
            ...
            base.forEach(e => {
                ...
                if (cd1) {
                    ...
                    //let toleranc = parseInt(i1[0].tolerancia)
                    let precoper = parseInt(i2[0].valor_locacao)
                    ...
                    $.get(link[35], r => {
                        ...
                        cd2 ? ativa(e.id, e.quarto, e.datahora, e.valor, precoper) : ""
                    })
                }
            })
        }
...
function ativa(id, suite, hora, valor, valorlocacao) {
    let f = tick[`${suite}`]
    locado(suite, f[0], f[1], f[2])
    var dados = { datahora: hora, valor: valor, quarto: suite, tipo: "locado" }
    alterar(`${link[11]}${id}/`, dados)
    insereValor(suite, valorlocacao, "locacao")
}
